...
def main_movimientos_y_documentos():
    ...
    try:
        ...
        for i, expediente in enumerate(expedientes_a_procesar):
            ...
            # --- 4. Obtener Movimientos (Lógica de Fase 2) ---
            ...
            if movements:
                print(f"  > Se leyó el CSV existente: '{csv_filename}' ({len(movements)} movimientos).")
            else:
                print(f"  > CSV no encontrado. Descargando movimientos para {nro_expediente}...")
                try:
                    ...
                    if movements:
                        ...
                        utils.save_to_csv(
                            movements, csv_filename, subdirectory=config.MOVIMIENTOS_OUTPUT_DIR
                        )
                    ...
                    print(f"  > !!! ERROR (Movimientos) al procesar {nro_expediente}: {e}")
                    continue # Saltar al siguiente expediente
            
            
            # --- 5. Bucle de Descarga de Documentos (Nueva Lógica) ---
            
            if not movements:
                print(f"  > No hay movimientos para procesar en {nro_expediente}.")
                continue

            print(f"  > Iniciando descarga de textos de {len(movements)} movimientos...")
            document_count = 0
            ...
            for movement in movements:
                ...
                # Solo procesar si tiene un link_escrito (algunos pueden no tenerlo)
                if doc_url and doc_url.strip():
                    document_count += 1
                    
                    # Crear nombre de archivo correlativo
                    # zfill(2) asegura 01, 02, ..., 10, 11
                    correlative_id = str(document_count).zfill(2)
                    txt_filename = f"{nro_sanitized} - {caratula_sanitized} - {correlative_id}.txt"
                    txt_filepath = os.path.join(config.DOCUMENTOS_OUTPUT_DIR, txt_filename)

                    # (Mejora): Saltar si el TXT ya existe
                    if os.path.exists(txt_filepath):
                        print(f"    > Doc {correlative_id}: Ya existe '{txt_filename}', saltando.")
                        continue
                    
                    print(f"    > Doc {correlative_id}: Descargando '{movement.get('nombre_escrito', 'doc')}'...")
                    
                    try:
                        # Tarea de scraping del documento
                        document_content = scraper_tasks.scrape_document_content(session, doc_url)

                        if document_content:
                            # Guardar el contenido en .txt
                            utils.save_to_txt(
                                document_content, txt_filename, subdirectory=config.DOCUMENTOS_OUTPUT_DIR
                            )
                        else:
                            print(f"    > ADVERTENCIA: No se pudo extraer contenido del Doc {correlative_id}.")
                        
                        time.sleep(0.5) # Pausa cortés entre cada petición de documento

                    except Exception as e:
                        print(f"    > !!! ERROR (Documento {correlative_id}) en {nro_expediente}: {e}")
