...
def ejecutar_fase_descarga_publica(session, link_detalle_objetivo, username):
    """Ejecuta la extraccion y consolidacion en PDF de un expediente publico especifico."""
    ...
    expediente_data = next((e for e in expedientes if e.get("link_detalle") == link_detalle_objetivo), None)
    
    if not expediente_data:
        return f"Error: El expediente no se encontró en la base de datos."
    ...
    logger.info(f"Iniciando proceso para expediente público: {nombre_base}")
    
    r_detalle = session.get(link_detalle_objetivo)
    html_detalle = r_detalle.text
    
    if "<frame name=\"sup\"" in html_detalle.lower():
        movimientos = scraper_tasks.raspar_movimientos_de_expediente(session, expediente_data)
    else:
        ajax_params_base = parsers.parsear_detalle_para_ajax_params(html_detalle)
        if "exp_id" not in ajax_params_base:
            logger.error(f"No se pudieron extraer los params de AJAX para {nro_expediente}.")
            movimientos = []
        else:
            offset = 0
            mov_page_count = 1
            ...
            while True:
                ...
                if len(r_movs.text) < 200:
                    break
                movs_pagina = parsers.parsear_movimientos_de_ajax_html(r_movs.text, nro_expediente)
                if not movs_pagina:
                    break
            ...
            try:
                ...
                if datos_documento:
                    ...
                    for idx, adj in enumerate(datos_documento.get("adjuntos", [])):
                        nombre_adj = utils.limpiar_nombre_archivo(adj["nombre"]).lower().replace(".pdf", "").replace(".", "")
                        pdfs.append({
                            "url": adj["url"],
