...
def ejecutar_fase_3_documentos(session, username):
    ...
    for i, expediente in enumerate(expedientes_a_procesar):
        ...
        logger.info(f"--- Procesando Expediente {i + 1}/{total_expedientes}: {nro_expediente} ---")
                    ...
                    if datos_documento:
                        ...
                        if adjuntos:
                            for idx, adj in enumerate(adjuntos):
                                ...
                                nombre_base = nombre_orig.replace(".PDF", "").replace(".pdf", "")
                        ...
                        if pdfs_a_descargar:
                            ...
                            for pdf_info in pdfs_a_descargar:
                                ruta_pdf = os.path.join(ruta_carpeta_expediente, pdf_info["nombre"])
                                if not os.path.exists(ruta_pdf):
                                    logger.info(f"      > Descargando {pdf_info['tipo']}: {pdf_info['nombre']}")
                                    if scraper_tasks.descargar_archivo(session, pdf_info["url"], ruta_pdf):
                                        total_pdfs_descargados += 1
                                else:
                                    logger.info(f"      > {pdf_info['tipo']} ya existe, saltando: {pdf_info['nombre']}")
                    time.sleep(0.5)
        ...
        if os.path.exists(ruta_pdf_final):
            logger.info(f"  > PDF Consolidado '{nombre_pdf_final}' ya existe, saltando fusión.")
        elif archivos_existentes_en_carpeta:
            utils.fusionar_pdfs(ruta_carpeta_expediente, ruta_pdf_final)
        else:
            logger.warning(f"  > No hay PDFs para consolidar en {nro_expediente}.")
